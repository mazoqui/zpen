...
int ensure_zpen_directory()
{
  ...
  free(zpen_dir);
  // if (bgPixmap) XFreePixmap(d, bgPixmap);
  return 0;
}
...
int main()
{
  ...
  // Capture the current screen BEFORE creating and mapping our fullscreen window
  freeze_img = XGetImage(d, DefaultRootWindow(d), 0, 0, width, height, AllPlanes, ZPixmap);
  ...
  XSelectInput(d, w,
    ...
    // If we captured the screen, use it as the window background to "freeze" the desktop
  if (freeze_img) {
    bgPixmap = XCreatePixmap(d, DefaultRootWindow(d), width, height, XDefaultDepth(d, screen));
    GC gc_bg = XCreateGC(d, bgPixmap, 0, NULL);
    XPutImage(d, bgPixmap, gc_bg, freeze_img, 0, 0, 0, 0, width, height);
    XFreeGC(d, gc_bg);
    XSetWindowBackgroundPixmap(d, w, bgPixmap);
    XDestroyImage(freeze_img);
    freeze_img = NULL;
  }
...
// We show the window
  ...
  // Paint the background pixmap we set above
  XClearWindow(d, w);
